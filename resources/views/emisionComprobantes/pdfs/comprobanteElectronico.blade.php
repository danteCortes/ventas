...
<html lang='es'>
    <head>
        <title>
            <span>{{'Comprobante N° '.$venta_administrador->serie}}-{{$venta_administrador->correlativo}}</span>
        </title>
        <style>
            #invoice-POS {
    ...
    <body>
        <div id="invoice-POS">
            ...
            <div id="top" class="text-center">
                <?php
                    $logo = 'logo.png';
                    $tienda = \App\Tienda::find($venta_administrador->tienda_id);
                    if($tienda->logo){
                        $logo = 'storage/logos/'.$tienda->logo;
                    }
                ?>
                <img class="logo" src="./{{$logo}}" alt="Logo"
                    style="max-width: 50%; height: 80px;">
                <h1>{{$tienda->nombre}}</h1>
                <p class="font-size-11">{{$tienda->direccion}}</p>                
            </div>
            ...
            <div id="datos-empresa" class="text-center font-size-11">
                ...
                <p>RUC: 20601867835</p>
                @if($tienda->telefono)
                    <p>TELF: {{$tienda->telefono}}</p>
                @endif
            </div>

            <div id="comprobante" class="mtb-10 text-center font-bold">
                @if($venta_administrador->empresa_ruc)
                    <h2>FACTURA ELECTRONICA</h2>
                @else
                    <h2>BOLETA DE VENTA ELECTRONICA</h2>
                @endif
                <h2>N° {{$venta_administrador->serie}}-{{$venta_administrador->correlativo}}</h2>
            </div>
            ...
            <div id="datos-cliente" class="mtb-10 font-size-10">                
                <table style="width: 100%;">
                    <tr>
                        ...
                        <td>{{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $venta_administrador->fecha_emision)->format('d/m/Y')}}</td>
                    </tr>
                    <?php
                        if($empresa = \App\Empresa::find($venta_administrador->empresa_ruc)){

                            $cliente_denominacion = $empresa->nombre;
                            $tipo_documento = 'RUC';
                            $documento = $empresa->ruc;
                            $direccion = $empresa->direccion;
                        }elseif($persona = \App\Persona::find($venta_administrador->persona_dni)){
                    ...
                    <?php
                        $usuario = \App\Usuario::join('personas as p', 'p.dni', '=', 'usuarios.persona_dni')
                            ->select(\DB::raw("concat(p.nombres, ' ', p.apellidos) as nombres_apellidos"))
                            ->where('usuarios.id', $venta_administrador->usuario_id)
                            ->first()
                        ;
                    ?>
                    <tr>
            ...
            <div id="listado-productos" class="mtb-10 font-size-11">
                <table id="productos" style="width: 100%;">
                    ...
                    @foreach(\App\DetalleVentaAdministrador::where('venta_administrador_id', $venta_administrador->id)->get() as $item)
                        <tr>
                            ...
                            <td class="text-right">
                                {{number_format($item->valor_venta, 2, '.', ' ')}}
                            </td>
                        </tr>
                    @endforeach
                </table>
                ...
                <div id="cobro" class="mtb-10">
                    <table style="width: 100%;">
                        <tr>
                            <td style="width: 75%">OP. EXONERADA:</td>
                            <td class="text-right">
                                S/ {{number_format($venta_administrador->total, 2, '.', ' ')}}
                            </td>
                        </tr>
                        <tr>
                            <td style="width: 75%">IGV (18%):</td>
                            <td class="text-right">S/ 0.00</td>
                        </tr>
                        <tr class="text-bold">
                            <td style="width: 75%">TOTAL:</td>
                            <td class="text-right">S/ {{number_format($venta_administrador->total, 2, '.', ' ')}}</td>
                        </tr>
                    </table>
                ...
                <table class="mt-10" style="width: 100%;">
                    <tr>
                        ...
                        <td class="text-center">
                            <img class="qr" src="./storage/qrTemp.svg" alt="QR">
                        </td>
                    </tr>
                </table>
