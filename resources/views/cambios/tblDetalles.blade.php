    ...
    <div class="table-responsive">
      <table class="table table-condensed table-bordered" style="background-color:#bfbfbf;">
        ...
        <tbody id="detalles">
            ...
            @if($tarjetaVenta = $venta->tarjetaVenta)
              @if($tarjetaVenta->tarjeta->comision != 0)
                <tr>
                  ...
                  {{Form::close()}}</td>
                  <td>1</td>
                  <td colspan="2">COMISIÓN POR USO DE TARJETA {{$tarjetaVenta->tarjeta->nombre}} {{$tarjetaVenta->tarjeta->comision}}%</td>
          ...
          <tr>
            <td colspan="5"><strong class="pull-right">TOTAL VENTA: </strong></td>
            <td style="text-align:right">{{number_format($cambio->total_anterior, 2, '.', ' ')}}</td>
          </tr>
          <tr>
            <td colspan="5"><strong class="pull-right">DIFERENCIA: </strong></td>
