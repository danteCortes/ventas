...
StyleSwitcher.prototype.build = function () {
    ...
    var h5 = $('<h5 />').html('Cambiar Estilo').append(h5A);
    ...
    var styleSwitcherWrap = $('<div />')
        ...
        .append($('<h6 />').html('Colores de Fondo'), colorList, $('<hr/>'));
    ...
    var fontWhite = $('<label/>').addClass('btn btn-xs btn-primary').html('Blanco').append(fgwbtn);
    ...
    var fontBlack = $('<label/>').addClass('btn btn-xs btn-danger').html('Negro').append(fgbbtn);
    var fgBtnGroup = $('<div/>').addClass('btn-group').attr('data-toggle', 'buttons').append(fontWhite, fontBlack);
    styleSwitcherWrap.append($('<div/>').addClass('options-link').append($('<h6/>').html('Colores de fuente'), fgBtnGroup));
    ...
    var resetLink = $('<a/>').html('Restaurar').attr('href', '#').on(Metis.buttonPressedEvent, function (e) {
        $this.reset();
        e.preventDefault();
    });
