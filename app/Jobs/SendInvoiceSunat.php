...
class SendInvoiceSunat implements ShouldQueue
{
    ...
    {
        try{
            $certificado = Certificado::where('tienda_id', $this->comprobante->tienda_id)->first();
            $tienda = $this->comprobante->tienda;
    ...
    {
        if($empresa = $this->comprobante->recibo->empresa){
            $tipo_doc = '6';
            $documento = $empresa->ruc;
            $denominacion = $empresa->nombre;
        }elseif($persona = $this->comprobante->recibo->persona){
            $tipo_doc = '1';
    ...
    private function getDetalles()
    {
        ...
        foreach($this->comprobante->detalles as $detalle)
        {
            $item = (new SaleDetail())
                ...
                ->setMtoValorUnitario($detalle->precio_unidad)
                ->setDescripcion($detalle->producto->familia->nombre.' '.$detalle->producto->marca->nombre.' '.$detalle->producto->descripcion)
                ->setMtoBaseIgv($detalle->total)
                ->setPorcentajeIgv(18.00) // 18%
                ->setIgv(0)
                ->setTipAfeIgv('20') // Gravado Op. Onerosa - Catalog. 07
                ->setTotalImpuestos(0) // Suma de impuestos en el detalle
                ->setMtoValorVenta($detalle->total)
                ->setMtoPrecioUnitario($detalle->precio_unidad)
            ;
    ...
    private function getInvoice()
    {
        ...
        $invoice = (new Invoice())
            ...
            ->setTipoDoc($tipo_doc) // Factura - Catalog. 01 
            ->setSerie($serie)
            ->setCorrelativo($correlativo)
            ->setFechaEmision(new \DateTime($this->comprobante->updated_at)) // Zona horaria: Lima
            ->setFormaPago(new FormaPagoContado()) // FormaPago: Contado
            ->setTipoMoneda('PEN') // Sol - Catalog. 02
            ->setCompany($this->getEmpresa())
            ->setClient($this->getCliente())
            ->setMtoOperExoneradas($this->comprobante->total)
            ->setMtoIGV(0)
            ->setTotalImpuestos(0)
            ->setValorVenta($this->comprobante->total)
            ->setSubTotal($this->comprobante->total)
            ->setMtoImpVenta($this->comprobante->total)
        ;
        
        $legend = (new Legend())
            ->setCode('1000') // Monto en letras - Catalog. 52
            ->setValue($this->numtoletras($this->comprobante->total))
        ;
